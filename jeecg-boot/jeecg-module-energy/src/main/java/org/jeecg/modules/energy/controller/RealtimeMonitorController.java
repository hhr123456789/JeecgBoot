...
public class RealtimeMonitorController {
    ...
    public Result<List<ModuleInfoVO>> getModulesByDimension(
            @ApiParam(value = "维度编码", required = true, example = "A02A02A01") 
            @RequestParam String dimensionCode,
            @ApiParam(value = "能源类型(1:电力,2:天然气,3:压缩空气,4:企业用水)", required = true, example = "1") 
            @RequestParam Integer energyType,
            @ApiParam(value = "是否包含子维度", example = "true") 
        ...
        try {
            List<ModuleInfoVO> result = realtimeMonitorService.getModulesByDimension(dimensionCode, energyType, includeChildren);
    ...
    public Result<TimeSeriesResultVO> getTimeSeriesData(
            @ApiParam(value = "时序数据查询参数", required = true) 
            @Valid @RequestBody TimeSeriesQueryVO query) {
    ...
    public Result<List<ModuleStatusVO>> getCurrentStatus(
            @ApiParam(value = "实时状态查询参数", required = true) 
            @Valid @RequestBody CurrentStatusRequestVO request) {
        
        log.info("获取实时状态，仪表ID：{}，参数：{}", request.getModuleIds(), request.getParameters());
        
        try {
            List<ModuleStatusVO> result = realtimeMonitorService.getCurrentStatus(request.getModuleIds(), request.getParameters());
    ...
    public Result<LoadTimeSeriesResultVO> getLoadTimeSeriesData(
            @ApiParam(value = "负荷时序数据查询参数", required = true)
            @Valid @RequestBody LoadTimeSeriesQueryVO query) {
    ...
    public Result<List<ModuleLoadStatusVO>> getCurrentLoadStatus(
            @ApiParam(value = "实时负荷状态查询参数", required = true)
            @Valid @RequestBody LoadStatusRequestVO request) {
    ...
    public Result<LoadTableResultVO> getLoadTableData(
            @ApiParam(value = "负荷表格查询参数", required = true)
            @Valid @RequestBody LoadTableQueryVO query) {
